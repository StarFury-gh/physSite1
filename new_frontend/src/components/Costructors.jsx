...
export function TestConstructor() {
  ...
  const handleSubmit = () => {
    ...
    axios.post(`${API_URL}/add_test`, { author: sessionStorage.getItem("current_user"), tasks: tasks, answers: answers, title: testTitle })
      .then(r => {
        console.log(r)
      })
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-2">
      ...
      <input onChange={(e) => setTestTitle(e.target.value)} type="text" placeholder="Тема" className="border-1 border-neutral-300 px-3 py-1 rounded-xs" />
      {questions.map((q) => (
        <QuestionBlock
          key={q.id}
          id={q.id}
...
const QuestionBlock = ({ id, question, answer, onChange, onRemove }) => {
  return (
    <div className="bg-white border-1 border-neutral-300 px-2 py-2 rounded-xs">
