...
async def login(user, password) -> dict:
    ...
    if utils.isExists(picked_data):
        ...
        cursor.close()
        conn.close()
        return {"status": False, "code": 404, "info": "no user in DataBase"}
...
async def register(user, password, validation_code) -> dict:
    if validation_code == "teacher_account":
        ...
        cursor = conn.cursor()
        picked_data = cursor.execute(
            f"SELECT * FROM users WHERE (login='{user}')"
        ).fetchall()
        ...
        if utils.isExists(picked_data):
            ...
            cursor.execute(
                f"INSERT INTO users (login, password) VALUES ('{user}', '{password}')"
            )
            conn.commit()
            cursor.close()
            conn.close()
...
def get_teacher_by_id(id: int):
    conn = sqlite3.connect(config.db_name)
    cursor = conn.cursor()
    necessary_teacher = cursor.execute(
        f"SELECT login FROM users WHERE id={id}"
    ).fetchone()
...
def add_task(task: Task) -> dict:
    ...
    try:
        ...
        cursor.execute(
            f"INSERT INTO excercises (author, text, theme) VALUES ('{task_teacher}', '{task_text}', '{task_theme}')"
        )
        conn.commit()
        return {"status": True}
...
def get_tasks():
    try:
        ...
        cursor = conn.cursor()
        data = cursor.execute(
            "SELECT author, theme, text, id FROM excercises"
        ).fetchall()
...
def get_task_by_id(id):
    try:
        ...
        cursor = conn.cursor()
        task = cursor.execute(
            f"SELECT * FROM excercises WHERE (id={id})"
        ).fetchone()
        if task:
...
def get_tasks_by_teacher(teacher):
    ...
    conn = sqlite3.connect(config.db_name)
    cursor = conn.cursor()
    teachersName = cursor.execute(
        f"SELECT login FROM users WHERE id={int(teacher)}"
    ).fetchone()
    tasks = cursor.execute(
        f"SELECT id, theme FROM excercises WHERE (author='{teachersName[0]}')"
    ).fetchall()
